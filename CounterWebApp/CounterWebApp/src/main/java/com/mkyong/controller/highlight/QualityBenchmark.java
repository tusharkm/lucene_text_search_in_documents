...
public class QualityBenchmark {
	...
	/**
	 * index field to extract doc name for each search result; used for judging the
	 * results.
	 */
	protected String docNameField;

	/**
	 * maximal number of queries that this quality benchmark runs. Default: maxint.
	 ...
	 *            index to be searched.
	 * @param docNameField
	 *            name of field containing the document name. This allows to extract
	 *            the doc name for search results, and is important for judging the
	 *            results.
	 */
	public QualityBenchmark(QualityQuery qqs[], QualityQueryParser qqParser, Searcher searcher, String docNameField) {
	 ...
	 * 
	 * @param judge
	 *            the judge that can tell if a certain result doc is relevant for a
	 *            certain quality query. If null, no judgements would be made.
	...
	public QualityStats[] execute(Judge judge, SubmissionReport submitRep, PrintWriter qualityLog) throws Exception {
		...
		for (int i = 0; i < nQueries; i++) {
			...
			if (submitRep != null) {
				submitRep.report(qq, td, docNameField, searcher.indexSearcher);// (arg0, arg1, arg2, arg3);
																				// report(qq,td,docNameField,searcher);
			}
		}
		if (submitRep != null) {
			submitRep.flush();
	...
	private QualityStats analyzeQueryResults(QualityQuery qq, Query q, TopDocs td, Judge judge, PrintWriter logger,
		...
		long t1 = System.currentTimeMillis(); // extraction of first doc name we measure also construction of doc name
												// extractor, just in case.
